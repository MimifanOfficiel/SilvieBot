...
module.exports = async bot => {
    ...
    bot.commands.forEach(async command => {
        ...
        if(command.options?.length >= 1) {
            for(let i = 0; i < command.options.length; i++){
                 slashCommand[`add${command.options[i].type.slice(0,1).toUpperCase()
                        + command.options[i].type.slice(1, command.options[i].type.length)}Option`] (option => 
    ...
    console.log("Les slash commands ont été générées avec succès.");
}